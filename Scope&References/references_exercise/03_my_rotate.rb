...
# When given a positive 'amt', a single rotation will rotate right, causing the 
# first element to be moved to the back of the array
# When given a negative 'amt', a single rotation will rotate left, causing the 
# last element to be moved to the front of the array.
...
def my_rotate!(array, amt)
    ...
    if amt > 0
        amt.times do 
            char  = array.shift() # remove from front
            array.push(char) # add to the end
        end
    end
    if amt < 0
        (amt.abs).times do
            char  = array.pop() # remove from the end
            array.unshift(char) # add to the front
        end
    end
